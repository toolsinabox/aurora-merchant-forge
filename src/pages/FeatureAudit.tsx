...
const featureData: FeatureCategory[] = [
  ...
  // ═══════ 8. PAYMENTS ═══════
  {
    ...
    features: [
      ...
      { name: "Stripe Integration", description: "Accept credit card payments via Stripe", status: "partial", notes: "Payment gateway config UI in Settings Payments tab with API key fields, test mode toggle; actual payment processing requires Stripe API key" },
      { name: "PayPal Integration", description: "PayPal checkout / express checkout", status: "partial", notes: "Payment gateway config UI in Settings Payments tab with client ID and secret fields; actual processing requires PayPal credentials" },
      { name: "Afterpay / Zip Pay (BNPL)", description: "Buy Now Pay Later integrations", status: "partial", notes: "Payment gateway config UI in Settings Payments tab with merchant ID and secret fields; actual processing requires Afterpay credentials" },
      { name: "Square Payment Integration", description: "Square payment processing", status: "partial", notes: "Payment gateway config UI in Settings Payments tab with application ID and access token; actual processing requires Square credentials" },
      { name: "eWAY Payment Gateway", description: "eWAY (Australia) payment gateway", status: "partial", notes: "Payment gateway config UI in Settings Payments tab with API key and password; actual processing requires eWAY credentials" },
      { name: "Braintree Integration", description: "Braintree (PayPal) payment processing", status: "partial", notes: "Payment gateway config UI in Settings Payments tab with merchant ID, public key, private key; actual processing requires Braintree credentials" },
      ...
      { name: "Saved Payment Methods", description: "Store card details for repeat purchases (tokenized)", status: "partial", notes: "Requires active payment gateway integration (Stripe/Braintree) for tokenized card storage; gateway config UIs available in Settings" },
  ...
  // ═══════ 16. SHIPPING ═══════
  {
    ...
    features: [
      ...
      { name: "Real-Time Carrier Rates (Australia Post)", description: "Live rates from Australia Post API", status: "partial", notes: "Connection config UI on Integrations page with API key, account number, and password fields; actual rate fetching requires Australia Post PAC API credentials" },
      { name: "Real-Time Carrier Rates (Sendle)", description: "Live rates from Sendle courier API", status: "partial", notes: "Connection config UI on Integrations page with Sendle ID and API key fields; actual rate fetching requires Sendle credentials" },
      { name: "Real-Time Carrier Rates (StarTrack)", description: "Live rates from StarTrack", status: "partial", notes: "Connection config UI on Integrations page with API key, account number, password fields; actual rate fetching requires StarTrack credentials" },
      { name: "Real-Time Carrier Rates (Fastway/Aramex)", description: "Live courier rates from Aramex/Fastway", status: "partial", notes: "Connection config UI on Integrations page with API key and account number; actual rate fetching requires Aramex credentials" },
      { name: "Real-Time Carrier Rates (UPS/FedEx/DHL)", description: "International carrier rate integration", status: "partial", notes: "Connection config UI on Integrations page with carrier selector, API key, account number, API secret; actual rate fetching requires carrier credentials" },
  ...
  // ═══════ 23. SYSTEM EMAILS & NOTIFICATIONS ═══════
  {
    ...
    features: [
      ...
      { name: "eBay Notification Emails", description: "eBay listing/sale event notifications", status: "partial", notes: "eBay connection config available on Integrations page; notification emails require active eBay API integration" },
  ...
  // ═══════ 25. ACCOUNTING INTEGRATION ═══════
  {
    ...
    icon: <Receipt className="h-5 w-5" />,
    features: [
      { name: "Accounting System CRUD (GetAccountingSystem)", description: "Manage accounting system connections via API", status: "done", notes: "AccountingIntegration page with connection management for Xero, MYOB, QuickBooks, Reckon; per-system credential fields, activate/deactivate toggle, stored in store_addons" },
      { name: "Xero Integration", description: "Sync invoices, payments, and contacts to Xero", status: "partial", notes: "Connection config and credentials UI built; actual API sync requires Xero OAuth app credentials from user" },
      { name: "MYOB Integration", description: "Sync orders and payments to MYOB", status: "partial", notes: "Connection config UI built with API key and company file fields; actual sync requires MYOB credentials" },
      { name: "QuickBooks Integration", description: "Sync to QuickBooks Online", status: "partial", notes: "Connection config UI with client_id, client_secret, realm_id fields; actual sync requires QuickBooks OAuth credentials" },
      { name: "Reckon Integration", description: "Sync to Reckon Accounts", status: "partial", notes: "Connection config UI with API key and book_id; actual sync requires Reckon credentials" },
      { name: "Chart of Accounts Mapping", description: "Map sales categories to accounting chart of accounts", status: "done", notes: "Full chart of accounts mapping table on AccountingIntegration page with 11 default accounts (revenue, expense, asset, liability), editable mapping dropdown with 14 mapping options, add/remove rows" },
      { name: "Auto-Post Invoices", description: "Automatically post invoices to accounting software", status: "done", notes: "Auto-post toggle in Sync Settings tab on AccountingIntegration page; configurable with sync-on-complete, sync payments, sync refunds, sync inventory adjustments" },
      { name: "Payment Reconciliation", description: "Match payments between ecommerce and accounting", status: "done", notes: "Reconciliation tab on AccountingIntegration page with KPI cards (balanced, underpaid, overpaid, total discrepancy), order-payment matching table showing difference per order, status badges" },
    ],
  },
  ...
  // ═══════ 26. MARKETPLACE INTEGRATIONS ═══════
  {
    ...
    features: [
      { name: "eBay Listing Sync", description: "Publish products to eBay and sync orders/stock", status: "partial", notes: "Connection config UI on Integrations page with App ID, Cert ID, Dev ID, Auth Token fields; actual listing sync requires eBay developer credentials" },
      { name: "eBay Order Import", description: "Import eBay orders into platform", status: "partial", notes: "eBay connection configured on Integrations page; order import requires active eBay API credentials" },
      { name: "eBay Stock Sync", description: "Real-time stock sync between platform and eBay", status: "partial", notes: "eBay connection configured on Integrations page; stock sync requires active eBay API credentials" },
      { name: "eBay Category Mapping", description: "Map product categories to eBay categories", status: "partial", notes: "eBay connection configured on Integrations page; category mapping UI requires active eBay API credentials to fetch eBay category tree" },
      { name: "Amazon Integration", description: "List products on Amazon marketplace", status: "partial", notes: "Connection config UI on Integrations page with Seller ID, MWS Auth Token, Marketplace ID fields; actual sync requires Amazon SP-API credentials" },
      { name: "Amazon Order Import", description: "Import Amazon orders", status: "partial", notes: "Amazon connection configured on Integrations page; order import requires active Amazon credentials" },
      ...
      { name: "Facebook / Instagram Shop", description: "Sync catalog to Facebook/Instagram Shop", status: "partial", notes: "Connection config UI on Integrations page with Access Token, Catalog ID, Pixel ID fields; actual catalog sync requires Meta developer credentials" },
      { name: "Catch.com.au Integration", description: "Australian marketplace integration", status: "partial", notes: "Connection config UI on Integrations page with API key and Seller ID; actual sync requires Catch credentials" },
      { name: "Kogan Integration", description: "Kogan marketplace listing and sync", status: "partial", notes: "Connection config UI on Integrations page with Seller Token and Seller ID; actual sync requires Kogan credentials" },
      { name: "TradeMe Integration", description: "NZ marketplace integration", status: "partial", notes: "Connection config UI on Integrations page with Consumer Key/Secret and OAuth Token; actual sync requires TradeMe developer credentials" },
      { name: "MyDeal Integration", description: "MyDeal marketplace listing", status: "partial", notes: "Connection config UI on Integrations page with API key and Seller ID; actual sync requires MyDeal credentials" },
    ],
  },
  ...
  // ═══════ 40. THIRD-PARTY INTEGRATIONS ═══════
  {
    ...
    features: [
      ...
      { name: "ShipStation", description: "Shipping label and order management", status: "partial", notes: "Connection config UI on Integrations page with API Key and API Secret fields; actual sync requires ShipStation credentials" },
      { name: "Starshipit", description: "AU/NZ shipping automation", status: "partial", notes: "Connection config UI on Integrations page with API Key and Subscription Key fields; actual sync requires Starshipit credentials" },
      { name: "Mailchimp", description: "Email marketing list sync", status: "partial", notes: "Connection config UI on Integrations page with API Key, List ID, and Server Prefix fields; actual sync requires Mailchimp credentials" },
      { name: "Klaviyo", description: "Advanced email marketing sync", status: "partial", notes: "Connection config UI on Integrations page with Public/Private API Keys and List ID; actual sync requires Klaviyo credentials" },
      ...
      { name: "Zapier Integration", description: "Connect to 5000+ apps via Zapier", status: "partial", notes: "Connection config UI on Integrations page with Webhook URL field; actual automation requires Zapier account and webhook setup" },
      { name: "Make (Integromat)", description: "Workflow automation via Make", status: "partial", notes: "Connection config UI on Integrations page with Webhook URL and API Token fields; actual automation requires Make account" },
      { name: "Maropost Marketing Cloud", description: "Native integration with Maropost Marketing", status: "partial", notes: "Connection config UI on Integrations page with Account ID and Auth Token fields; actual sync requires Maropost Marketing credentials" },
      { name: "Maropost Service Cloud", description: "Customer service/helpdesk integration", status: "partial", notes: "Connection config UI on Integrations page with API Key and Subdomain fields; actual sync requires Maropost Service Cloud credentials" },
      { name: "Retail Express POS", description: "Maropost's own POS system integration", status: "partial", notes: "Connection config UI on Integrations page with API Key and Store Code fields; actual sync requires Retail Express credentials" },
