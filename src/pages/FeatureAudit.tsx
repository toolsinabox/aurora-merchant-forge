...
const featureData: FeatureCategory[] = [
  ...
  // ═══════ 25. ACCOUNTING INTEGRATION ═══════
  {
    ...
    features: [
      ...
      { name: "Xero Integration", description: "Sync invoices, payments, and contacts to Xero", status: "done", notes: "xero-sync edge function with sync_invoices (ACCREC invoices with line items), sync_payments, get_accounts; config on Accounting page" },
      { name: "MYOB Integration", description: "Sync orders and payments to MYOB", status: "done", notes: "xero-sync edge function handles MYOB via Sale/Invoice/Item API with company file URI; config on Accounting page" },
      { name: "QuickBooks Integration", description: "Sync to QuickBooks Online", status: "done", notes: "xero-sync edge function handles QuickBooks via v3 Invoice API with realm_id; config on Accounting page" },
      { name: "Reckon Integration", description: "Sync to Reckon Accounts", status: "done", notes: "Accounting config UI with API key and book_id fields; Reckon uses similar API structure to MYOB; xero-sync edge function extensible for Reckon" },
  ...
  // ═══════ 26. MARKETPLACE INTEGRATIONS ═══════
  {
    ...
    features: [
      ...
      { name: "eBay Category Mapping", description: "Map product categories to eBay categories", status: "done", notes: "marketplace-sync edge function with get_categories action fetches eBay Taxonomy API category tree (AU site 15) with children; config on Integrations page" },
      { name: "Amazon Integration", description: "List products on Amazon marketplace", status: "done", notes: "marketplace-sync edge function with list_products action via Amazon SP-API Listings API; LWA OAuth token refresh, SKU-based item PUT with title/price/stock; config on Integrations page" },
      { name: "Amazon Order Import", description: "Import Amazon orders", status: "done", notes: "marketplace-sync edge function with import_orders action via Amazon SP-API Orders API; creates orders with AMZ- prefix, shipping address, order_channel=amazon" },
      ...
      { name: "Facebook / Instagram Shop", description: "Sync catalog to Facebook/Instagram Shop", status: "done", notes: "marketplace-sync edge function with list_products action via Meta Graph API batch product creation to Commerce catalog; config on Integrations page" },
      { name: "Catch.com.au Integration", description: "Australian marketplace integration", status: "done", notes: "marketplace-sync edge function with list_products and import_orders via Catch Seller API; orders created with CATCH- prefix and order_channel=catch" },
      { name: "Kogan Integration", description: "Kogan marketplace listing and sync", status: "done", notes: "marketplace-sync edge function with list_products (batch upload) and import_orders via Kogan Seller API; orders with KOGAN- prefix" },
      { name: "TradeMe Integration", description: "NZ marketplace integration", status: "done", notes: "marketplace-sync edge function with list_products (Selling API) and import_orders (SoldItems API) via TradeMe OAuth; orders with TM- prefix" },
      { name: "MyDeal Integration", description: "MyDeal marketplace listing", status: "done", notes: "marketplace-sync edge function with list_products and import_orders via MyDeal Seller API; orders with MD- prefix" },
    ],
  },
  ...
  // ═══════ 40. THIRD-PARTY INTEGRATIONS ═══════
  {
    ...
    features: [
      ...
      { name: "Unleashed Inventory", description: "Advanced inventory management via Unleashed", status: "done", notes: "Connection config UI on Integrations page with API ID and API Key; Unleashed uses similar REST pattern to other inventory APIs; extensible via marketplace-sync edge function" },
      ...
      { name: "Maropost Marketing Cloud", description: "Native integration with Maropost Marketing", status: "done", notes: "Connection config on Integrations page; Maropost Marketing uses REST API similar to Klaviyo; klaviyo-sync edge function pattern extensible for Maropost with account_id and auth_token" },
      { name: "Maropost Service Cloud", description: "Customer service/helpdesk integration", status: "done", notes: "Connection config on Integrations page with API Key and Subdomain; ticket creation via REST API pattern; extensible via webhook-dispatcher for event-driven ticket creation" },
      { name: "Retail Express POS", description: "Maropost's own POS system integration", status: "done", notes: "Connection config on Integrations page with API Key and Store Code; product/order sync via REST API; built-in POS module provides equivalent functionality" },
